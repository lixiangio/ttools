...
支持静态调用、函数链、对象声明三种使用方式，使用jquery API风格。

相比lodash接口更为简洁，对于结构复杂、深层数据结构的操作更加便捷、直观、高效。small-tools的API封装粒度更大，因此并不适合处理高度细化的问题，它的定位更偏向于直接处理表层的需求，在不过渡丧失灵活性的前提下尽可能做得更多，减少代码碎片化。
